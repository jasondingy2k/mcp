...
test('persistClipboard: darwin exec 成功但未写文件 → ClipboardError 且无残留', async () => {
  try {
    unlinkSync(FAKE_OUT);
  } catch {}
  await assert.rejects(
    ...
    (e) =>
      e instanceof ClipboardError &&
      /valid image file from clipboard/i.test(e.message) &&
      /卡在 剪贴板读取/.test(e.message) &&
      !/损坏|截断|corrupt/i.test(e.message)
  );
  assert.equal(existsSync(FAKE_OUT), false, '空落盘失败后临时文件应被清理');
});

test('persistClipboard: win32 exec 成功但 0 字节 → ClipboardError 且无残留', async () => {
  try {
    unlinkSync(FAKE_OUT);
  } catch {}
  await assert.rejects(
...
test('persistClipboard: linux 明确暂不支持且不建临时路径', async () => {
  ...
  await assert.rejects(
    ...
    (e) => {
      if (!(e instanceof ClipboardError && /not supported/.test(e.message))) return false;
      message = e.message;
      return true;
    }
  );
