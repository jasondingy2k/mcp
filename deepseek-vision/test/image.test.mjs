...
test('prepareImageForModel: 大图最长边缩至 ≤2048 且 mime 为 png', async () => {
  const large = await sharp({
    create: { width: 3000, height: 2000, channels: 3, background: { r: 10, g: 20, b: 30 } },
  })
    .png()
    .toBuffer();
  const { buffer, mime } = await prepareImageForModel(large);
  const meta = await sharp(buffer).metadata();
  assert.ok(Math.max(meta.width ?? 0, meta.height ?? 0) <= 2048);
  assert.equal(mime, 'png');
});

test('prepareImageForModel: 小图原样返回不重编码', async () => {
  const { buffer, mime } = await prepareImageForModel(TINY_PNG);
  assert.ok(buffer.equals(TINY_PNG));
  assert.equal(mime, 'png');
});

test('prepareImageForModel: VISION_MAX_SEND_EDGE=0 时大图不缩小', async () => {
  const prev = process.env.VISION_MAX_SEND_EDGE;
  process.env.VISION_MAX_SEND_EDGE = '0';
  try {
    const large = await sharp({
      ...
      .toBuffer();
    const { buffer, mime } = await prepareImageForModel(large);
    assert.ok(buffer.equals(large));
    assert.equal(mime, 'png');
  } finally {
    if (prev === undefined) delete process.env.VISION_MAX_SEND_EDGE;
    else process.env.VISION_MAX_SEND_EDGE = prev;
  }
});
