// 零成本单元测试：共享模块（.env 加载器 / 错误前缀 / 日志工厂）。
// 运行：npm install && npm run build && node --test
...
test('makeToolError: 前缀格式', () => {
  const toolError = makeToolError('test-server');
  assert.equal(toolError('TypeX', 'msg'), '[test-server 内部错误] TypeX: msg');
});
